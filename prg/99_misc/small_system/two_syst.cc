...
public:
  ...
  void season(animal* a)
  {
    ...
    population -= a->get_pop()*0.5; //*0.01 / (this->strength / a->get_str() );
    */
...
public:
  ...
  void season(animal * a)
  {
    ...
    /*
    population *= (1 + this->reproduction - this->mortality);
    population += a->get_pop()*0.1; //*(.01 * this->strength / a->get_str());
    */
    if(population < 0) population = 0;
    return;
  }
...
int main()
{
  // population(P),reproduction(R),mortality(M),strength(S)
  //              P     R      M     S  
  predator cats ( 100,  0.,  0.01 , 0.5  );
  prey mice     (100,  0.45,  0.01,   0.1);
